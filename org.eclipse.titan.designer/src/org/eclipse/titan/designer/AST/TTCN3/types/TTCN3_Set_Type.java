...
public final class TTCN3_Set_Type extends TTCN3_Set_Seq_Choice_BaseType {
		...
		for (int i = 0; i < nofValueComponents; i++) {
			...
			if (!realComponents.containsKey(valueId.getName())) {
				namedValue.getLocation().reportSemanticError(
						MessageFormat.format(NONEXISTENTFIELDTTCN3, namedValue.getName().getDisplayName(),
								getTypename()));
			} else {
	...
	public void generateCode( final JavaGenData aData, final StringBuilder source ) {
		...
		RecordSetCodeGenerator.generateValueClass(aData, source, className, classReadableName, namesList, hasOptional, true, hasRaw, raw, hasJson, jsonAsValue, jsonAsMapPossible, localTypeDescriptor);
