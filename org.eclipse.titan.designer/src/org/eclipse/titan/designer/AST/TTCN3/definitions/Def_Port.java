...
public final class Def_Port extends Definition {
	...
	public void addProposal(final ProposalCollector propCollector, final int index) {
		...
		} else if (subrefs.size() > index + 1 && portType != null && identifier.getName().equals(subrefs.get(index).getId().getName())) {
			// perfect match
			portType.addProposal(propCollector, index + 1);
		}
	}
	...
	public void addDeclaration(final DeclarationCollector declarationCollector, final int index) {
		...
		if (subrefs.size() > index && identifier.getName().equals(subrefs.get(index).getId().getName())) {
			if (subrefs.size() > index + 1 && portType != null) {
				portType.addDeclaration(declarationCollector, index + 1);
