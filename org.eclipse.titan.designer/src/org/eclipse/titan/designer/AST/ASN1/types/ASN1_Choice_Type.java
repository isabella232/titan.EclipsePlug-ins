...
public final class ASN1_Choice_Type extends ASN1_Set_Seq_Choice_BaseType {
	...
	public String generateConversion(final JavaGenData aData, final IType fromType, final String fromName, final ExpressionStruct expression) {
		...
		case TYPE_TTCN3_CHOICE:{
			...
			return generateConversionTTCNChoiceToASNChoice(aData, realFromType, fromName, expression);
		}
		case TYPE_ASN1_CHOICE:{
			//heavy conversion is needed
			final ASN1_Choice_Type realFromType = (ASN1_Choice_Type) refdType;
			return generateConversionASNChoiceToASNChoice(aData, realFromType, fromName, expression);
		}
		default:
	...
	protected String generateConversionTTCNChoiceToASNChoice(final JavaGenData aData, final TTCN3_Choice_Type fromType, final String fromName, final ExpressionStruct expression) {
	...
	protected String generateConversionASNChoiceToASNChoice(final JavaGenData aData, final ASN1_Choice_Type fromType, final String fromName, final ExpressionStruct expression) {
		final String tempId = aData.getTemporaryVariableName();
		final String name = getGenNameValue(aData, expression.preamble);
		expression.preamble.append(MessageFormat.format("final {0} {1} = new {0}();\n", name, tempId));
		final String ConversionFunctionName = Type.getConversionFunction(aData, fromType, this, expression.preamble);
