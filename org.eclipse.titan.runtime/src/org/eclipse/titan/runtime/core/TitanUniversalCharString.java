...
public class TitanUniversalCharString extends Base_Type {
	...
	public static final TTCN_JSONdescriptor TitanUniversalCharString_json_ = new TTCN_JSONdescriptor(false, null, false, null, null, false, false, false, 0, null, false, json_string_escaping.ESCAPE_AS_SHORT);
	public static final TTCN_Typedescriptor TitanUniversalCharString_descr_ = new TTCN_Typedescriptor("universal charstring", null, TitanUniversalCharString_raw_, TitanUniversalCharString_json_, null);
	...
	public int JSON_encode(final TTCN_Typedescriptor p_td, final JSON_Tokenizer p_tok, final boolean p_parent_is_map) {
		if (!is_bound()) {
			TTCN_EncDec_ErrorContext.error(TTCN_EncDec.error_type.ET_UNBOUND, "Encoding an unbound universal charstring value.");
			return -1;
		}
	...
	public int JSON_decode(final TTCN_Typedescriptor p_td, final JSON_Tokenizer p_tok, final boolean p_silent, final boolean p_parent_is_map, final int p_chosen_field) {
		...
		if (json_token_t.JSON_TOKEN_ERROR == token.get()) {
			...
			if (TitanCharString.from_JSON_string(value.toString(), !use_default, out)) {
				...
				charstring = false;
				// value contains UTF-8 encoded byte sequence, this is not a printable Java string
	...
	private boolean from_JSON_string(final boolean check_quotes) {
		...
		final List<TitanUniversalChar> ustr = new ArrayList<TitanUniversalChar>();
		boolean error = false;
		...
		for (int i = start; i < end; ++i) {
			if (json_str.get(i).is_char() && '\\' == json_str.get(i).getUc_cell()) {
				...
				case 'n':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\n'));
					break;
				case 't':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\t'));
					break;
				case 'r':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\r'));
					break;
				case 'f':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\f'));
					break;
				case 'b':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\b'));
					break;
				case '\\':
					ustr.add(new TitanUniversalChar((char)0, (char)0, (char)0, '\\'));
					break;
					...
					if (end - i >= 6 && json_str.get(i + 2).is_char() && json_str.get(i + 3).is_char() &&
						...
						if (row_upper_nibble <= 0x0F && row_lower_nibble <= 0x0F &&
							...
							ustr.add(new TitanUniversalChar((char)0, (char)0,
									(char) ((row_upper_nibble << 4) | row_lower_nibble),
									(char) ((cell_upper_nibble << 4) | cell_lower_nibble)));
