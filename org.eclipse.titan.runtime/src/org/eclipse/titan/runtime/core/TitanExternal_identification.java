...
public class TitanExternal_identification extends Base_Type {
	...
	public int JSON_encode(final TTCN_Typedescriptor p_td, final JSON_Tokenizer p_tok) {
		...
		case ALT_fixed:
			...
			break;
		default:
			TTCN_EncDec_ErrorContext.error(TTCN_EncDec.error_type.ET_UNBOUND, "Encoding an unbound value of type EXTERNAL.identification.");
			return -1;
		}
		...
		if (null != p_td.json && p_td.json.isAs_value()) {
			...
			if (json_token_t.JSON_TOKEN_NAME != j_token.get()) {
				...
				if (8 == name_len.get() && "syntaxes".equals(fld_name.toString())) {
					int ret_val = get_field_syntaxes().JSON_decode(TitanExternal_identification_syntaxes_descr_, p_tok, p_silent);
					if (0 > ret_val) {
					...
					int ret_val = get_field_context__negotiation().JSON_decode(TitanExternal_identification_context__negotiation_descr_, p_tok, p_silent);
					if (0 > ret_val) {
						if (JSON.JSON_ERROR_INVALID_TOKEN == ret_val) {
