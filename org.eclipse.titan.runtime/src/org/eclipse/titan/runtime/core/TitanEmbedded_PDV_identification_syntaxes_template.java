...
public class TitanEmbedded_PDV_identification_syntaxes_template extends Base_Template {
	...
	public TitanEmbedded_PDV_identification_syntaxes_template operator_assign(final Base_Type otherValue) {
		...
		throw new TtcnError(MessageFormat.format("Internal Error: value `{0}'' can not be cast to `TitanEmbedded_PDV_identification_syntaxes''", otherValue));
	}
	...
	public TitanEmbedded_PDV_identification_syntaxes_template operator_assign(final Base_Template otherValue) {
		...
		throw new TtcnError(MessageFormat.format("Internal Error: value `{0}'' can not be cast to `TitanEmbedded_PDV_identification_syntaxes_template''", otherValue));
	}
	 ...
	 * @param other_value the value to be copied.
	 * */
	private void copy_template(final TitanEmbedded_PDV_identification_syntaxes_template other_value) {
	...
	public int n_list_elem() {
		if (template_selection != template_sel.VALUE_LIST && template_selection != template_sel.COMPLEMENTED_LIST) {
			throw new TtcnError("Internal error: Accessing a list element of a non-list template of enumeration type EMBEDDED PDV.identification.syntaxes.");
		}
		return list_value.size();
	}

	@Override
	public TitanEmbedded_PDV_identification_syntaxes_template list_item(final int list_index) {
		if (template_selection != template_sel.VALUE_LIST && template_selection != template_sel.COMPLEMENTED_LIST) {
	...
	public void check_restriction(final template_res restriction, final String name, final boolean legacy) {
		...
		throw new TtcnError(MessageFormat.format("Restriction `{0}'' on template of type {1} violated.", get_res_name(restriction), name == null ? "EMBEDDED PDV.identification.syntaxes" : name));
	}
}