...
public class TitanEmbedded_PDV_identification_context__negotiation_template extends Base_Template {
	 ...
	 * */
	public TitanEmbedded_PDV_identification_context__negotiation_template(final template_sel otherValue ) {
		super( otherValue );
	 ...
	 *                the value to initialize to.
	 * */
	public TitanEmbedded_PDV_identification_context__negotiation_template( final Optional<TitanEmbedded_PDV_identification_context__negotiation> otherValue ) {
	...
	@Override
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign( final template_sel otherValue ) {
		check_single_selection(otherValue);
		clean_up();
	 ...
	 */
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign( final TitanEmbedded_PDV_identification_context__negotiation otherValue ) {
		clean_up();
		copy_value(otherValue);
		return this;
	}
	 ...
	 * @return the new template object.
	 */
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign( final TitanEmbedded_PDV_identification_context__negotiation_template otherValue ) {
		if (otherValue != this) {
			clean_up();
			copy_template(otherValue);
		}
	...
	@Override
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign(final Base_Type otherValue) {
	...
	@Override
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign(final Base_Template otherValue) {
		if (otherValue instanceof TitanEmbedded_PDV_identification_context__negotiation_template) {
			return operator_assign((TitanEmbedded_PDV_identification_context__negotiation_template) otherValue);
		}
	 ...
	 * @return the new template object.
	 */
	public TitanEmbedded_PDV_identification_context__negotiation_template operator_assign( final Optional<TitanEmbedded_PDV_identification_context__negotiation> otherValue ) {
		clean_up();
		switch (otherValue.get_selection()) {
		case OPTIONAL_PRESENT:
			copy_value(otherValue.constGet());
	...
	private void copy_value(final TitanEmbedded_PDV_identification_context__negotiation other_value) {
	...
	private void copy_template(final TitanEmbedded_PDV_identification_context__negotiation_template other_value) {
	...
	public boolean match(final TitanEmbedded_PDV_identification_context__negotiation other_value, final boolean legacy) {
		...
		case SPECIFIC_VALUE:
			...
			if(!presentation__context__id.match(other_value.get_presentation__context__id(), legacy)) {
				return false;
			}
			if(!other_value.get_transfer__syntax().is_bound()) {
				return false;
			}
			if(!transfer__syntax.match(other_value.get_transfer__syntax(), legacy)) {
				return false;
			}
			return true;
		case VALUE_LIST:
	...
	@Override
	public TitanEmbedded_PDV_identification_context__negotiation_template list_item(final int list_index) {
		...
		if ( TTCN_Logger.matching_verbosity_t.VERBOSITY_COMPACT == TTCN_Logger.get_matching_verbosity() ) {
			if(match(match_value, legacy)) {
				...
				if (template_selection == template_sel.SPECIFIC_VALUE) {
					...
					if( !presentation__context__id.match(match_value.constGet_presentation__context__id(), legacy) ) {
						TTCN_Logger.log_logmatch_info(".presentation-context-id");
						presentation__context__id.log_match(match_value.constGet_presentation__context__id(), legacy);
						TTCN_Logger.set_logmatch_buffer_len(previous_size);
					}
					if( !transfer__syntax.match(match_value.constGet_transfer__syntax(), legacy) ) {
						TTCN_Logger.log_logmatch_info(".transfer-syntax");
						transfer__syntax.log_match(match_value.constGet_transfer__syntax(), legacy);
						TTCN_Logger.set_logmatch_buffer_len(previous_size);
					}
				} else {
					TTCN_Logger.print_logmatch_buffer();
	...
	public void decode_text(final Text_Buf text_buf) {
		...
		case COMPLEMENTED_LIST: {
			...
			list_value = new ArrayList<TitanEmbedded_PDV_identification_context__negotiation_template>(size);
	...
	public void set_param(final Module_Parameter param) {
		...
		case MP_Value_List:
			if (param.get_size() > 2) {
				param.error(MessageFormat.format("record template of type EMBEDDED PDV.identification.context-negotiation has 2 fields but list value has {0} fields.", param.get_size()));
			}
	...
	public void check_restriction(final template_res restriction, final String name, final boolean legacy) {
		...
		case TR_VALUE:
			...
			this.presentation__context__id.check_restriction(restriction, name == null ? "EMBEDDED PDV.identification.context-negotiation" : name, legacy);
			this.transfer__syntax.check_restriction(restriction, name == null ? "EMBEDDED PDV.identification.context-negotiation" : name, legacy);
			return;
		case TR_PRESENT:
		...
		throw new TtcnError(MessageFormat.format("Restriction `{0}'' on template of type {1} violated.", get_res_name(restriction), name == null ? "EMBEDDED PDV.identification.context-negotiation" : name));
	}
}