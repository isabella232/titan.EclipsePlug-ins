...
public class TitanCharacter_String_template extends Base_Template {
	...
	private void copy_value(final TitanCharacter_String other_value) {
		...
		if (other_value.get_field_string__value().is_bound()) {
			get_field_string__value().operator_assign(other_value.get_field_string__value());
		} else {
			get_field_string__value().clean_up();
		}
	...
	private void copy_template(final TitanCharacter_String_template other_value) {
		...
		case SPECIFIC_VALUE:
			...
			if (template_sel.UNINITIALIZED_TEMPLATE == other_value.get_field_string__value().get_selection()) {
				get_field_string__value().clean_up();
			} else {
				get_field_string__value().operator_assign(other_value.get_field_string__value());
			}
	 ...
	 * */
	public TitanCharacter_String_identification_template get_field_identification() {
		set_specific();
	 ...
	 * @return the field identification.
	 * */
	public TitanCharacter_String_identification_template constGet_field_identification() {
		if (template_selection != template_sel.SPECIFIC_VALUE) {
	 ...
	 * */
	public TitanOctetString_template get_field_string__value() {
		set_specific();
	 ...
	 * @return the field string-value.
	 * */
	public TitanOctetString_template constGet_field_string__value() {
		if (template_selection != template_sel.SPECIFIC_VALUE) {
	...
	private void set_specific() {
		if (template_selection != template_sel.SPECIFIC_VALUE) {
			...
			if (old_selection == template_sel.ANY_VALUE || old_selection == template_sel.ANY_OR_OMIT) {
				identification = new TitanCharacter_String_identification_template(template_sel.ANY_VALUE);
				data__value__descriptor = new TitanUniversalCharString_template(template_sel.ANY_OR_OMIT);
				string__value = new TitanOctetString_template(template_sel.ANY_VALUE);
			} else {
	...
	public boolean match(final TitanCharacter_String other_value, final boolean legacy) {
		...
		case SPECIFIC_VALUE:
			...
			if(!string__value.match(other_value.get_field_string__value(), legacy)) {
				return false;
			}
			return true;
		case VALUE_LIST:
	...
	public TitanCharacter_String valueof() {
		...
		if (string__value.is_bound()) {
			ret_val.get_field_string__value().operator_assign(string__value.valueof());
		}
		return ret_val;
	}
		...
		if ( TTCN_Logger.matching_verbosity_t.VERBOSITY_COMPACT == TTCN_Logger.get_matching_verbosity() ) {
			if(match(match_value, legacy)) {
				...
				if (template_selection == template_sel.SPECIFIC_VALUE) {
					...
					if( !string__value.match(match_value.constGet_field_string__value(), legacy) ) {
