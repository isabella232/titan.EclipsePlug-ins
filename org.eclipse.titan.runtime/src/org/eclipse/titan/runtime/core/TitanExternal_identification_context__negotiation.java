...
public class TitanExternal_identification_context__negotiation extends Base_Type {
	...
	public void set_param(final Module_Parameter param) {
		...
		case MP_Value_List:
			if (param.get_size() > 2) {
				param.error(MessageFormat.format("record value of type EXTERNAL.identification.context-negotiation has 2 fields but list value has {0} fields.", param.get_size()));
			}
		...
		case MP_Assignment_List: {
			...
			for (int val_idx = 0; val_idx < param.get_size(); val_idx++) {
				if (!value_used[val_idx]) {
					param.get_elem(val_idx).error(MessageFormat.format("Non existent field name in type EXTERNAL.identification.context-negotiation: {0}", param.get_elem(val_idx).get_id().get_name()));
					break;
				}
			}
			break;
