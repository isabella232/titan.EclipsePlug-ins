...
public class TitanInteger_template extends Base_Template {
	...
	public Module_Parameter get_param(final Module_Param_Name param_name) {
		...
		case ANY_OR_OMIT:
			...
			break;
		case SPECIFIC_VALUE:
			mp = new Module_Param_Integer(single_value);
			break;
		case VALUE_LIST:
		case COMPLEMENTED_LIST: {
		...
		case VALUE_RANGE: {
			final TitanInteger lower_bound = min_is_present ? min_value : null;
			final TitanInteger upper_bound = max_is_present ? max_value : null;
			mp = new Module_Param_IntRange(lower_bound, upper_bound, min_is_exclusive, max_is_exclusive);
			break;
		}
		default:
