...
public class TitanBitString_template extends Restricted_Length_Template {
	...
	private static int[] pattern_string_2_list(final String patternString) {
	...
	private static int pattern_char_2_byte(final char patternChar) {
	...
	private static String pattern_list_2_string(final int[] list) {
		if (list == null) {
			throw new TtcnError("Internal error: bitstring pattern list is null.");
		}
		final StringBuilder result = new StringBuilder(list.length);
	 ...
	 *                use legacy mode.
	 * */
	public boolean match(final TitanBitString_Element otherValue, final boolean legacy) {
		return match(new TitanBitString(otherValue), legacy);
	}
	...
	public Module_Parameter get_param( final Module_Param_Name param_name ) {
		...
		case STRING_PATTERN: {
			mp = new Module_Param_Bitstring_Template(pattern_list_2_string(pattern_value));
			break;
		}
		case DECODE_MATCH:
