...
public class TitanExternal_identification_syntaxes extends Base_Type {
	...
	public int JSON_encode(final TTCN_Typedescriptor p_td, JSON_Tokenizer p_tok) {
		if (!is_bound()) {
			TTCN_EncDec_ErrorContext.error(TTCN_EncDec.error_type.ET_UNBOUND, "Encoding an unbound value of type EXTERNAL.identification.syntaxes.");
			return -1;
		}
				...
				if (8 == name_len.get() && "abstract".equals(fld_name.toString())) {
					...
					if (0 > ret_val) {
						if (JSON.JSON_ERROR_INVALID_TOKEN == ret_val) {
							JSON_ERROR(p_silent, error_type.ET_INVAL_MSG, JSON.JSON_DEC_FIELD_TOKEN_ERROR, 8, "abstract");
						}
						return JSON.JSON_ERROR_FATAL;
					}
					dec_len += ret_val;
					...
					if (0 > ret_val) {
						if (JSON.JSON_ERROR_INVALID_TOKEN == ret_val) {
							JSON_ERROR(p_silent, error_type.ET_INVAL_MSG, JSON.JSON_DEC_FIELD_TOKEN_ERROR, 8, "transfer");
						}
						return JSON.JSON_ERROR_FATAL;
					}
					dec_len += ret_val;
					...
					JSON_ERROR(p_silent, error_type.ET_INVAL_MSG, JSON.JSON_DEC_INVALID_NAME_ERROR, name_len, fld_name);
