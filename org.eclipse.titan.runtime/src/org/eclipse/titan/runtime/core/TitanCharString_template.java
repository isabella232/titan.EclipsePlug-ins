...
public class TitanCharString_template extends Restricted_Length_Template {
	...
	public Module_Parameter get_param(final Module_Param_Name param_name) {
		...
		case VALUE_RANGE: {
			final TitanUniversalChar lower_bound = new TitanUniversalChar( (char)0, (char)0, (char)0, min_value.get_value().charAt(0) );
			final TitanUniversalChar upper_bound = new TitanUniversalChar( (char)0, (char)0, (char)0, max_value.get_value().charAt(0) );
			mp = new Module_Param_StringRange(lower_bound, upper_bound, min_is_exclusive, max_is_exclusive);
			break;
		}
		case STRING_PATTERN:
			mp = new Module_Param_Pattern(single_value.get_value().toString(), pattern_value_nocase);
			break;
		case DECODE_MATCH:
