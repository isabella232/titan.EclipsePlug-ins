...
public class TitanEmbedded_PDV_template extends Base_Template {
	...
	public TitanBoolean match(final TitanEmbedded_PDV other_value) {
		return match(other_value, false);
	}

	public TitanBoolean match(final TitanEmbedded_PDV other_value, final boolean legacy) {
		return new TitanBoolean(match_(other_value, legacy));
	}

	private boolean match_(final TitanEmbedded_PDV other_value, final boolean legacy) {
		...
		if ( TtcnLogger.matching_verbosity_t.VERBOSITY_COMPACT == TtcnLogger.get_matching_verbosity() ) {
			if(match(match_value, legacy).getValue()) {
				...
				if (templateSelection == template_sel.SPECIFIC_VALUE) {
					...
					if( !data__value.match(match_value.constGetData__value(), legacy).getValue() ) {
